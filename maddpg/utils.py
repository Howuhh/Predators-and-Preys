...
class ReplayBuffer:
    def __init__(self, size, n_agents, state_size, action_size=1, device="cpu"):
        ...
        self.agents_states = [torch.empty((size, state_size), dtype=torch.float, device=device) for _ in range(n_agents)]
        self.agents_actions = [torch.empty((size, action_size), dtype=torch.float, device=device) for _ in range(n_agents)]
        self.agents_rewards = [torch.empty((size, 1), dtype=torch.float, device=device) for _ in range(n_agents)]
        self.agents_next_states = [torch.empty((size, state_size), dtype=torch.float, device=device) for _ in range(n_agents)]
    ...
    def add(self, agents_states, global_state, agents_actions, agents_rewards, agents_next_states, global_next_state, done):
        ...
        for agent_idx in range(self.n_agents):
            self.agents_states[agent_idx][self._buffer_idx] = torch.tensor(agents_states[agent_idx], device=self.device)
            ...
            self.agents_next_states[agent_idx][self._buffer_idx] = torch.tensor(agents_next_states[agent_idx], device=self.device)
    ...
    def sample(self, batch_size):
        ...
        agents_states = [agent_states[idxs] for agent_states in self.agents_states]
        ...
        agents_next_states = [agent_next_states[idxs] for agent_next_states in self.agents_next_states]
